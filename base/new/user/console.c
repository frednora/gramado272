...
//
    ...
    if ( CONSOLE_TTYS[console_number].cursor_left >= CONSOLE_TTYS[console_number].cursor_right )
    {
        panic ("console_oubyte: l >= r ");
    }

    // Sem espaço vertical
    if ( CONSOLE_TTYS[console_number].cursor_top >= CONSOLE_TTYS[console_number].cursor_bottom )
    {
...
// Caso contrario, apenas incrementa a coluna.
    ...
    if ( CONSOLE_TTYS[console_number].cursor_x >= (CONSOLE_TTYS[console_number].cursor_right -1) )
    {
        CONSOLE_TTYS[console_number].cursor_x = CONSOLE_TTYS[console_number].cursor_left;
        CONSOLE_TTYS[console_number].cursor_y++;
    }else{ 
...
// de limite diferente desse.
    ...
    { 
        ...
        CONSOLE_TTYS[console_number].cursor_x = 0; //CONSOLE_TTYS[console_number].cursor_left;
        CONSOLE_TTYS[console_number].cursor_y = (CONSOLE_TTYS[console_number].cursor_bottom -1);
    }
...
draw:
    xxxConsoleOutbyte (Ch, console_number);
    prev = Ch;
}
...
void xxxConsoleOutbyte (int c, int console_number)
{
    ...
    if ( console_number < 0 || console_number >= CONSOLETTYS_COUNT_MAX  )
    {
        debug_print ("xxxConsoleOutbyte: [FAIL] console_number\n");
        x_panic     ("xxxConsoleOutbyte: [FAIL] console_number\n");
    }
